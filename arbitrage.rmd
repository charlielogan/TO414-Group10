...
6. `borrower_rate` is interest rate owed on that particular loan
...
tr3 <- subset(tr, select= -c(first_recorded_credit_month, borrower_state, current_delinquencies,
                         ...
                         current_delinquencies, monthly_debt, months_employed))   #exclude insignificant vars
...
#trl$dti_wprosper_loan_adj <- ifelse(trl$dti_wprosper_loan>10, NA, listings$dti_wprosper_loan) # remove outliers
#trl$monthly_debt_adj <- ifelse(trl$monthly_debt>13000, NA, listings$monthly_debt)# remove outliers
...
tr4 <- subset(tr_noNA, select= c(defaulted, number_of_days, principal_balance, amount_funded ,listing_term ,prosper_score , income_range ,occupation, monthly_debt_adj, monthly_debt, public_records_last12_months ,bankcard_utilization  ,total_open_revolving_accounts,total_inquiries,loan_origination_month))
...
defaulted ~ number_of_days + principal_balance + amount_funded + listing_term + prosper_score + income_range + monthly_debt + public_records_last12_months + total_open_revolving_accounts + total_inquiries + loan_origination_month
```{r}
...
summary(step_newmodel)   #AIC: 13898
...
step_pred <- as.factor(ifelse(predict(step_newmodel, newdata = log_xtest,  type = "response") >= 0.5, 1, 0))
table(step_pred)  

confusionMatrix(data = step_pred, reference = log_ytest)  # 86.8% Accuracy
...
#AIC to beat: 13898
lm1 <- glm(formula = defaulted ~ amount_funded + listing_term + prosper_score + 
    income_range + monthly_debt + public_records_last12_months + 
    bankcard_utilization + total_open_revolving_accounts + 
    loan_origination_month, family = "binomial", data = tr4)
summary(lm1) #13898

lm2 <- glm(formula = defaulted ~ amount_funded + listing_term + prosper_score + 
    income_range + monthly_debt + public_records_last12_months + 
    bankcard_utilization + total_open_revolving_accounts, family = "binomial", data = tr4)
summary(lm2) #13903



```
defaulted ~ amount_funded + listing_term + prosper_score + 
    income_range + monthly_debt + public_records_last12_months + 
    bankcard_utilization + total_open_revolving_accounts + 
...
```{r}
#AIC to beat 13898
im1 <- glm(defaulted ~   amount_funded + 
listing_term + prosper_score + income_range + monthly_debt + 
public_records_last12_months + bankcard_utilization + total_open_revolving_accounts + loan_origination_month + amount_funded*monthly_debt + bankcard_utilization*total_open_revolving_accounts, data = tr4, family = "binomial")

summary(im1) #AIC: 13885


#Accuracy from step to beat: 86.8%

im1_pred <- as.factor(ifelse(predict(im1, newdata = log_xtest,  type = "response") >= 0.5, 1, 0))

confusionMatrix(data = im1_pred, reference = log_ytest)   # Accuracy = 86.8%
```
```{r}
im2 <- glm(defaulted ~   amount_funded + 
listing_term + prosper_score + income_range + monthly_debt_adj + 
public_records_last12_months + bankcard_utilization + total_open_revolving_accounts + loan_origination_month + amount_funded*monthly_debt_adj + bankcard_utilization*total_open_revolving_accounts + `monthly_debt_adj`*`bankcard_utilization`, data = tr4, family = "binomial")

summary(im2) #AIC: 13976


#Accuracy to beat: 86.8%


im2_pred <- as.factor(ifelse(predict(im2, newdata = log_xtest,  type = "response") >= 0.5, 1, 0))

confusionMatrix(data = im2_pred, reference = log_ytest)   # Accuracy = 86.6%
```
```{r}
im3 <- glm(defaulted ~   amount_funded + 
listing_term + prosper_score + income_range + monthly_debt_adj + 
public_records_last12_months + bankcard_utilization + total_open_revolving_accounts + loan_origination_month + amount_funded*monthly_debt_adj + bankcard_utilization*total_open_revolving_accounts + monthly_debt_adj*bankcard_utilization + income_range*I(total_open_revolving_accounts)^2, data = tr4, family = "binomial")

summary(im3) #AIC: 13973


im3_pred <- as.factor(ifelse(predict(im3, newdata = log_xtest,  type = "response") >= 0.5, 1, 0))

confusionMatrix(data = im3_pred, reference = log_ytest)   # Accuracy = 86.6%
```
    
Column Title Notes:
`defaulted` True if loan defaulted or charged-off
`amount_funded` Initial Invested Amount minus total principal payments made to the Lender so far
`listing_term` Loan pay-off maturity
`prosper_score` Estimated probability of loan going "bad" (60+ days past due within 1st year)
`income_range` Of borrower, as determined by lender
`monthly_debt` Regular monthly debt obligation (minimum payment)
`public_records_last12_months` Includes bankruptcy, forclosure, tax evasion, lawsuits, etc
`bankcard_utilization` Total debt utilized by borrowed, out of total revolving credit approved for by Lenders
`total_open_revolving_accounts` Max credit limit to borrow against, without specific maturity date.

Variables with significant interactions:
`bankcard_utilization` * `total_open_revolving_accounts`
`amount_funded` *`monthly_debt_adj`
`monthly_debt_adj` * `bankcard_utilization`
`income_range` * `total_open_revolving_accounts`    " Coefficients: (1 not defined because of singularities)  "

```{r}

# Plot interaction terms to visualize
plot(listings$total_open_revolving_accounts,listings$bankcard_utilization) # does not appear correlated
plot(listings$monthly_debt_adj, listings$amount_funded) # does not appear correlated. Amount funded tends to be a round figure.
plot(listings$income_range,listings$total_open_revolving_accounts) #might be a quadratic fit?
plot(listings$monthly_debt_adj,listings$bankcard_utilization) # does not appear correlated
```

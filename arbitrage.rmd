...
Combined the significant columns from both models and ran stepwise
...
tr3 <- subset(tr, select= -c(first_recorded_credit_month, borrower_state, current_delinquencies,
                         ...
                         current_delinquencies, monthly_debt, months_employed, loan_status))   #exclude insignificant vars
...
```{r}
lm4 <- lm(formula = borrower_rate ~ number_of_days + principal_balance + 
...
```
```{r}
lm5<- lm(formula = borrower_rate ~ number_of_days + principal_balance + 
    amount_funded + prosper_rating + listing_term + 
    listing_monthly_payment + prosper_score + income_verifiable + 

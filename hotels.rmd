...
In the following chunk "arrive", we generate a frequency distribution to display # arrivals by Month.
...
Guests book the fewest number of stays to arrive during the winter months of January and December. Hotels may consider events or promotions to promote bookings during these slower months.

### Arrivals by Week
In the following chunk "arr_by_wk" we plot how many bookings arrive each week as a bar chart, with colors transitioning by month. 
...
This barchart combines the data we have already seen to follow arrivals frequency over time.
******Identify some peak vs dog weeks. The hotel may investigate these patterns to prepare accordingly. Ex. May need to hire tem workers, supplies, parking overflow, logisitical cosiderations
...
We can see that the highest price is charged when there is no deposit. 

We wish to explore whether higher prices are charged in peak seasons.
...
In the next chunk "adr_boxplots", we will visualize the movement in this distribution by plotting vertical boxplots against week number.
...
barplot(sort(cityhotelprice, decreasing = TRUE)[1:20])
...
barplot(sort(resorthotelprice, decreasing = TRUE)[1:20])
```

Which companies have the highest number of bookings for City Hotel?

```{r echo=FALSE}
barplot(sort(table(h_city$company), decreasing = TRUE)[2:21])
```

Which companies have the highest number of bookings for Resort Hotel?

```{r echo=FALSE}
barplot(sort(table(h_resort$company), decreasing = TRUE)[2:21])
```
...
barplot(sort(cityhotelagentprice, decreasing = TRUE)[1:20])
```
...
barplot(sort(resorthotelagentprice, decreasing = TRUE)[1:20])
```

Which agents have the highest number of bookings for City Hotel?

```{r echo=FALSE}
barplot(sort(table(h_city$agent), decreasing = TRUE)[2:21])
```

Which agents have the highest number of bookings for Resort Hotel?

```{r echo=FALSE}
barplot(sort(table(h_resort$agent), decreasing = TRUE)[2:21])
```
...
canceledbyhotel
```
City hotel has a higher cancellation rate, where about 42% of reservations are canceled, while only about 28% are cancelled at Resort Hotel. 


We can compare the number of cancellations by people from each country on a graph:
```{r echo=FALSE}
hotels_bookings %>% group_by(country) %>% summarise(num_cancel = sum(is_canceled)) %>% arrange(desc(num_cancel)) %>% head(10) %>% print %>% ggplot(aes(x = country, y = num_cancel, fill= num_cancel))+ geom_col()
...
barplot(sort(table(hotels_bookings$arrival_date_month)), las=2)
```
...
hotels_bookings %>% group_by(market_segment) %>% summarise(num_cancel = mean(is_canceled)) %>% arrange(desc(num_cancel)) %>% ggplot(aes(x = market_segment, y = num_cancel, fill= num_cancel))+ geom_col()
```

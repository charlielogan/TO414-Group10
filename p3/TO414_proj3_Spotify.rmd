...
step_dm1_results <- predict(step_dm1,newdata = audio_testMM, type="response")
...
```{r}
dm2 <- lm(danceability ~ duration_ms + explicit + energy + key0 + key1 + key2 + key3 + key4 + key5 + key7 + key9 + loudness + mode + speechiness + acousticness + instrumentalness + liveness + valence +  I(tempo^2)+ tempo+ time_signature + release_year, data=audio_trainMM)
...
knn_test <- subset(log_test, select=-c(artists, popularity, year, hit_class, key))

...
* __Overarching Objective:__ Our goal is to better understand music consumption patterns and trends among popular songs through the years.
...
\n
**Descriptive Analytics:** How do audio features differ across decades? How do audio features differ by popularity? 
**Predictive Analytics:** Can we predict danceability? Can we predict popularity? Can we predict if a song is from a particular decade? Can we find patterns among a single artist's most popular songs?
...
# Creating popularity bins using popularity threshold of 50
...
#Building the logistic regression model using only significant variables
...
```{r}
#Predicting popularity using test set
...
## Artist Insights 
One of the limitations of the models we did on overall popularity, is that we only took into account the audio features that the dataset provided. In reality, there are many other things that go into the popularity of a song such as artist recognition, lyrics, marketing, awards, etc. In an attempt to better determine which audio features impact popularity, regardless of the artist, we tried to build models to understand what makes a specific artist’s (Taylor Swift’s) songs popular. The models can also be used to predict the popularity of Taylor Swift’s future songs. This is invaluable for Taylor Swift’s record label because they can tailor future songs according to the model’s results to increase the likelihood of popularity and thus potentially increase royalties (revenue). 


Overall, this project provided us with many different insights on the audio features of songs and their corresponding danceability and popularity scores. Specific stakeholders such as music producers, artists, advertisers, and music platform executives can use this analysis to extract information about how music is consumed and experienced to meet the needs of their clients and/or audiences.
